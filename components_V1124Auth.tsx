...
export default function V1124Auth({ returnTo = "/" }: Props) {
  ...
  async function startTelegramLink() {
    ...
    try {
      ...
      if (!r.ok) throw new Error(d.error || "TELEGRAM CONNECTION FAILED.");
      ...
      pollRef.current = setInterval(async () => {
        try {
          const statusResponse = await fetch("/api/auth", {
            method: "POST",
            headers: { "content-type": "application/json" },
            body: JSON.stringify({ action: "telegram-link-status", linkId: d.linkId }),
            cache: "no-store",
          });
          ...
          if (!statusResponse.ok) {
            if (pollRef.current) clearInterval(pollRef.current);
            setLinking(false);
            setError(statusData.error || "TELEGRAM CONNECTION FAILED.");
            return;
          }
      ...
      setLinking(false);
      setError(x instanceof Error ? x.message : "TELEGRAM CONNECTION FAILED.");
    }
  }
  ...
  async function submit(e: FormEvent) {
    ...
    if (mode === "signup") {
      ...
      if (!telegram?.telegramId || telegram.status !== "LINKED" || !linkId) {
    ...
    try {
      ...
      window.location.href = mode === "signup" ? "/" : returnTo;
    } catch (x) {
  ...
  async function recover(e: FormEvent) {
    ...
    try {
      const r = await fetch("/api/auth", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ action: "recovery", username }),
      });

      const d = await r.json();
      if (!r.ok) throw new Error(d.error || "RECOVERY REQUEST FAILED.");

      setSent(true);
    } catch (x) {
      setError(x instanceof Error ? x.message : "RECOVERY REQUEST FAILED.");
    } finally {
      setBusy(false);
    }
  }

  const signupReady = !!username && password.length >= 8 && confirm.length >= 8 && password === confirm;
          ...
          {mode === "choice" && !recovery ? (
            ...
            <form onSubmit={submit} className="space-y-3.5">
              <label className="block">
                <span className="text-[9px] font-black tracking-[0.16em] text-white/55">USERNAME</span>
                <input
                  value={username}
              ...
              <label className="block">
                <span className="text-[9px] font-black tracking-[0.16em] text-white/55">PASSWORD</span>
                <input
                  value={password}
              ...
              {mode === "signup" && (
                <>
                  <label className="block">
                    <span className="text-[9px] font-black tracking-[0.16em] text-white/55">CONFIRM PASSWORD</span>
                    <input
                      value={confirm}
                  ...
                  {!telegram ? (
                    <div className="pt-3">
                      <button
                        ...
                        className="mt-4 flex w-full items-center justify-center gap-3 rounded-xl border border-white bg-[#229ED9] px-5 py-4 text-xs font-black tracking-[0.14em] text-white disabled:cursor-not-allowed disabled:border-transparent disabled:opacity-40"
                      >
                        ...
                        {linking ? "CONNECTING TO TELEGRAM…" : "CONNECT TO TELEGRAM"}
                      </button>
                    </div>
                  ) : (
                    <div className="space-y-3 pt-3">
                      <div className="rounded-xl border border-[#229ED9]/40 bg-[#229ED9]/10 px-4 py-3">
                        <p className="text-[9px] font-black tracking-[0.16em] text-[#229ED9]">✓ TELEGRAM CONNECTED</p>
                      </div>

                      <label className="block">
                        <span className="text-[9px] font-black tracking-[0.16em] text-white/55">TELEGRAM ID</span>
                        <input
                      ...
                      <label className="block">
                        <span className="text-[9px] font-black tracking-[0.16em] text-white/55">TELEGRAM USERNAME</span>
                        <input
                          value={telegram.telegramUsername ? `@${telegram.telegramUsername.replace(/^@/, "")}` : "(NO USERNAME)"}
                          readOnly
                          className="mt-2 w-full rounded-xl border border-white/10 bg-[#090b0b] px-4 py-3 text-white/70 outline-none"
                        />
                      </label>
...
SIGN IN WITH IT AND CHANGE YOUR PASSWORD IMMEDIATELY.
                ...
                <>
                  ...
                  <button
                    ...
                    className="mt-4 w-full rounded-xl bg-white px-5 py-4 text-xs font-black tracking-[0.18em] text-black"
                  >
                    {busy ? "PLEASE WAIT…" : "SUBMIT RECOVERY REQUEST"}
                  </button>
                </>
              )}
