...
async function launch(browserName, url, incognito=false) {
  ...
  if (old) {
    try {
      if (old.incognito === incognito && !old.context.pages()[0]?.isClosed()) {
        const page = old.context.pages()[0] ?? await old.context.newPage();
  ...
  const chromiumOptions = {
    ...
    args: ["--window-position=80,60", "--window-size=1440,900"],
  };

  let context;
  if (incognito) {
    if (browserName === "firefox") {
      const browser = await firefox.launch({headless:false, viewport:null, downloadsPath:browserDownloadDir(browserName)});
      context = await browser.newContext({viewport:null, acceptDownloads:true});
    } else if (browserName === "edge") {
      const browser = await chromium.launch({...chromiumOptions, channel:"msedge"});
      context = await browser.newContext({viewport:null, acceptDownloads:true});
    } else {
    ...
    context = await firefox.launchPersistentContext(profileDir(browserName), {headless:false, viewport:null, acceptDownloads:true, downloadsPath:browserDownloadDir(browserName)});
  } else if (browserName === "edge") {
    context = await chromium.launchPersistentContext(profileDir(browserName), {...chromiumOptions, channel:"msedge"});
  } else {
    context = await chromium.launchPersistentContext(profileDir(browserName), chromiumOptions);
  }
  ...
  const captureDownload = async download => {
    try {
      ...
      if (downloadPath) await queueBrowserDownload(browserName, session, downloadPath, suggestedName);
    } catch {}
  };
  context.on("page", newPage => newPage.on("download", captureDownload));
  page.on("download", captureDownload);
  startBrowserDownloadWatcher(browserName, session);
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === "POST" && req.url === "/checker/ultimate") {
      ...
      const session = sessions.get(browser);
      if (!session || session.page.isClosed()) {
        return json(res, 409, { error: "No live Investigation Browser session." });
      }
      ...
      void runUltimateChecker(session.page, job)
        .then(result => {
          job.status = "COMPLETE";
