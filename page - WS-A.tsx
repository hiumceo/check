  ...
  return (
      ...
      {mounted && leavePrompt ? createPortal(
        <div role="dialog" aria-modal="true" className="fixed inset-0 z-[2147483647] flex items-center justify-center bg-black/70 p-4">
          <div className="w-full max-w-md rounded-2xl border border-cyan-400/20 bg-[#020608] p-5 text-white shadow-[0_0_40px_rgba(0,220,255,.18)]">
            <p className="text-xs font-black tracking-[0.2em] text-cyan-400">UNSAVED CHANGES</p>
            <h3 className="mt-2 text-xl font-black">You have unsaved changes.</h3>
            <p className="mt-2 text-xs leading-5 text-cyan-100/50">What would you like to do before leaving?</p>
            <div className="mt-5 grid grid-cols-2 gap-2">
              <button type="button" onClick={() => void leaveWithSave()} className="rounded-xl bg-cyan-400 px-3 py-3 text-[10px] font-black tracking-[0.12em] text-black">SAVE</button>
              <button type="button" onClick={() => setLeavePrompt(null)} className="rounded-xl border border-white/15 bg-black px-3 py-3 text-[10px] font-black tracking-[0.12em] text-white">CANCEL</button>
            </div>
          </div>
    ...
    {mounted && signOutPrompt ? createPortal(
      <div role="dialog" aria-modal="true" className="fixed inset-0 z-[2147483647] flex items-center justify-center bg-black/70 p-4">
        <div className="w-full max-w-md rounded-2xl border border-cyan-400/20 bg-[#020608] p-5 text-white shadow-[0_0_40px_rgba(0,220,255,.18)]">
          <p className="text-xs font-black tracking-[0.2em] text-cyan-400">UNSAVED CHANGES</p>
          <h3 className="mt-2 text-xl font-black">SAVE BEFORE SIGNING OUT?</h3>
          <p className="mt-2 text-xs leading-5 text-cyan-100/50">You have unsaved account changes. Save them before signing out so nothing is lost.</p>
          <div className="mt-5 grid grid-cols-2 gap-2">
            <button type="button" onClick={() => void saveAndSignOut()} disabled={signingOut || accountSaving} className="rounded-xl border border-cyan-400/30 bg-cyan-400 px-3 py-3 text-[10px] font-black tracking-[0.12em] text-black disabled:opacity-50">{signingOut ? "SAVING..." : "SAVE"}</button>
            <button type="button" onClick={() => setSignOutPrompt(false)} disabled={signingOut} className="rounded-xl border border-white/15 bg-black px-3 py-3 text-[10px] font-black tracking-[0.12em] text-white disabled:opacity-50">CANCEL</button>
          </div>
        </div>
      </div>, document.body) : null}
    {mounted && tierInfo ? createPortal(
      <div role="dialog" aria-modal="true" className="fixed inset-0 z-[2147483647] flex items-center justify-center bg-black/70 p-4">
        <div className="w-full max-w-md rounded-2xl border border-cyan-400/20 bg-[#020608] p-5 text-white shadow-[0_0_40px_rgba(0,220,255,.18)]">
          <p className="text-xs font-black tracking-[0.2em] text-cyan-400">ACCOUNT TIER</p>
          <h3 className="mt-2 text-xl font-black">{ACCOUNT_TIER_LABELS[tierInfo].toUpperCase()}</h3>
          <p className="mt-2 text-xs leading-5 text-cyan-100/50">Tier access is assigned by the system/admin. This option is shown so you can see what the tier offers and how progression works; it does not change your account tier.</p>
          <div className="mt-5 rounded-xl border border-cyan-400/15 bg-black/40 p-4">
            <p className="text-[10px] font-black tracking-[0.16em] text-cyan-400">STAR LEVEL</p>
            <p className="mt-2 text-sm font-black">{tierInfo === "NOVICE" ? "LEVEL 1–2" : tierInfo === "AMATEUR" ? "LEVEL 3" : "LEVEL 4–5"}</p>
            <p className="mt-3 text-[10px] leading-5 text-cyan-100/45">{tierInfo === "NOVICE" ? "Foundation access for learning the WorkStation workflow and building verified testing progress." : tierInfo === "AMATEUR" ? "The advancement tier for learning the broader WorkStation workflow before moving into Pro." : "The highest progression tier with the broadest WorkStation access."}</p>
          </div>
          <div className="mt-4 rounded-xl border border-cyan-400/15 bg-black/40 p-4">
            <p className="text-[10px] font-black tracking-[0.16em] text-cyan-400">HOW TO ADVANCE</p>
            <p className="mt-2 text-[10px] leading-5 text-cyan-100/45">Progress through verified testing activity and platform performance. The tier is assigned when the required level is reached or granted by the administrator.</p>
          </div>
          <button type="button" onClick={() => setTierInfo(null)} className="mt-5 w-full rounded-xl border border-white/15 bg-black px-3 py-3 text-[10px] font-black tracking-[0.12em] text-white">CLOSE</button>
        </div>
      </div>, document.body) : null}
    <DeleteConfirmModal
    ...
    <main className="min-h-screen overflow-hidden bg-[#020608] text-white">
      ...
      <div className="relative mx-auto min-h-screen max-w-6xl px-5 py-8 sm:px-10 sm:py-10">
        <div className="relative">
          ...
          <button type="button" aria-label="Open user profile" onClick={() => navigateAway("/workstation/user")} className="block h-11 w-11 shrink-0 overflow-hidden rounded-full border border-cyan-400/30 bg-cyan-400/10 shadow-[0_0_20px_rgba(0,220,255,.08)]" style={{ width: 44, height: 44, minWidth: 44, minHeight: 44, maxWidth: 44, maxHeight: 44 }}>
            {<img src={account.profile.avatar || "/favicon.png"} alt="Profile" className="block h-full w-full max-h-full max-w-full object-cover" style={{ width: "100%", height: "100%", maxWidth: "100%", maxHeight: "100%" }} />}
          </button>
          </div>
          <header className="mx-auto max-w-4xl text-center">
            <img src="/bug-workstation-logo.png" alt="v1124 Bug WorkStation" className="mx-auto h-auto w-full max-w-sm object-contain" />
            ...
            <p className="mt-4 text-[10px] font-black tracking-[0.16em] text-cyan-400 sm:text-xs">INVESTIGATION BROWSER &nbsp;|&nbsp; <span className="text-green-400">ULTIMATE CHECKER</span> &nbsp;|&nbsp; SIGNALS &nbsp;|&nbsp; EXTRACTORS &nbsp;|&nbsp; EVIDENCES &nbsp;|&nbsp; SOURCE HIERARCHY &nbsp;|&nbsp; PROJECT INSTRUCTION</p>
          </header>
        </div>
        ...
        <section className="mx-auto mt-6 max-w-5xl rounded-[28px] border border-cyan-400/30 bg-black/70 p-5 shadow-[0_0_40px_rgba(0,220,255,.05)] sm:p-7">
          <div className="grid grid-cols-2 gap-2">
            {LAYERS.map((item, index) => <button key={item.id} type="button" onClick={() => setLayer(item.id)} className={`rounded-xl border px-3 py-3 text-[9px] font-black tracking-[0.1em] transition ${layer === item.id ? "border-white/20 bg-white text-black" : "border-cyan-400/15 bg-cyan-400/[0.03] text-cyan-100/50 hover:text-cyan-100/80"}`}><span className="mr-2">●</span>{item.label}</button>)}
          </div>

          <div className="mt-6 rounded-2xl border border-cyan-400/20 bg-cyan-400/[0.025] p-5">
            {layer === "account" && <>
              <p className="text-xs font-black tracking-[0.22em] text-cyan-400">ACCOUNT SETTINGS</p>
              <h2 className="mt-2 text-2xl font-black">PROFILE</h2>
              <div className="mt-6 rounded-2xl border border-cyan-400/20 bg-black/40 p-5">
                <div className="grid gap-5 sm:grid-cols-[120px_1fr] sm:items-start">
                  <div className="text-center">
                    ...
                    <label className="mt-3 block cursor-pointer rounded-xl border border-cyan-400/15 bg-black/30 px-2 py-2 text-[9px] font-black tracking-[0.1em] text-cyan-300">CHANGE IMAGE<input type="file" accept="image/*" className="hidden" onChange={uploadAvatar} /></label>
                  </div>
                  <div className="grid gap-4">
                    <label className="block"><span className="text-[9px] font-black tracking-[0.14em] text-cyan-400/70">USERNAME</span><input value={username} onChange={e => { setUsername(e.target.value); setUsernameDirty(true); }} className="mt-2 w-full rounded-xl border border-cyan-400/15 bg-black px-3 py-3 text-sm outline-none" /></label>
                    <label className="block"><span className="text-[9px] font-black tracking-[0.14em] text-cyan-400/70">TELEGRAM ID</span><input value={authUser?.telegramId || ""} readOnly aria-readonly="true" className="mt-2 w-full cursor-not-allowed rounded-xl border border-cyan-400/10 bg-white/[0.03] px-3 py-3 text-sm text-cyan-100/55 outline-none" /></label>
                    <label className="block"><span className="text-[9px] font-black tracking-[0.14em] text-cyan-400/70">TELEGRAM USERNAME</span><input value={authUser?.telegramUsername ? `@${authUser.telegramUsername.replace(/^@/, "")}` : ""} readOnly aria-readonly="true" className="mt-2 w-full cursor-not-allowed rounded-xl border border-cyan-400/10 bg-white/[0.03] px-3 py-3 text-sm text-cyan-100/55 outline-none" /></label>
                  </div>
                </div>
              </div>

              <div className="mt-5 rounded-2xl border border-cyan-400/20 bg-black/40 p-5">
                <p className="text-xs font-black tracking-[0.22em] text-cyan-400">PASSWORD SETTINGS</p>
                <div className="mt-5 grid gap-4">
                  <label className="block"><span className="text-[9px] font-black tracking-[0.14em] text-cyan-400/70">CURRENT PASSWORD</span><input type="password" value={passwordCurrent} onChange={e => setPasswordCurrent(e.target.value)} autoComplete="current-password" className="mt-2 w-full rounded-xl border border-cyan-400/15 bg-black px-3 py-3 text-sm outline-none" /></label>
                  <label className="block"><span className="text-[9px] font-black tracking-[0.14em] text-cyan-400/70">NEW PASSWORD</span><input type="password" value={passwordNew} onChange={e => setPasswordNew(e.target.value)} autoComplete="new-password" className="mt-2 w-full rounded-xl border border-cyan-400/15 bg-black px-3 py-3 text-sm outline-none" /></label>
                  <label className="block"><span className="text-[9px] font-black tracking-[0.14em] text-cyan-400/70">CONFIRM NEW PASSWORD</span><input type="password" value={passwordConfirm} onChange={e => setPasswordConfirm(e.target.value)} autoComplete="new-password" className="mt-2 w-full rounded-xl border border-cyan-400/15 bg-black px-3 py-3 text-sm outline-none" /></label>
                </div>
                <button type="button" disabled={passwordSaving} onClick={() => void savePassword()} className="mt-5 rounded-xl border border-cyan-400/30 bg-cyan-400 px-4 py-3 text-[10px] font-black tracking-[0.12em] text-black disabled:cursor-not-allowed disabled:opacity-50">{passwordSaving ? "UPDATING..." : "UPDATE PASSWORD"}</button>
              </div>
            </>}

            {layer === "preferences" && <>
              <p className="text-xs font-black tracking-[0.22em] text-cyan-400">PREFERENCES</p><h2 className="mt-2 text-2xl font-black">WORKSPACE FEATURES PREFERENCES</h2>
              <div className="mt-6 rounded-2xl border border-cyan-400/30 bg-black/70 p-5">
                <div className="border-b border-cyan-400/20 pb-4"><p className="text-[10px] font-black tracking-[0.18em] text-cyan-400">ACCOUNT TIER</p><p className="mt-2 text-3xl font-black">{ACCOUNT_TIER_LABELS[account.tier].toUpperCase()}</p></div>
                <div className="mt-4 flex flex-wrap gap-2">{(["NOVICE","AMATEUR","PRO"] as Tier[]).map(tier => <button key={tier} type="button" disabled={account.tier === tier} onClick={() => setTierInfo(tier)} className={`rounded-xl border px-4 py-3 text-[10px] font-black tracking-[0.12em] transition ${account.tier === tier ? "cursor-default border-white/15 bg-white text-black" : "border-cyan-400/15 bg-cyan-400/[0.03] text-cyan-300 hover:border-cyan-300/40 hover:bg-cyan-400/10"}`}>{TIER_LABELS[tier].toUpperCase()}</button>)}</div>
              </div>
              <div className="mt-5 rounded-2xl border border-cyan-400/30 bg-black/70 p-5">
                <div className="border-b border-cyan-400/20 pb-4"><p className="text-[10px] font-black tracking-[0.18em] text-cyan-400">WORKSTATION LAYOUT</p><h3 className="mt-2 text-xl font-black">FEATURE VISIBILITY</h3><p className="mt-2 text-[10px] leading-5 text-cyan-100/45">Choose which Workspace features is available in your WorkStation. Features unavailable to your tier remain disabled.</p></div>
                <div className="mt-4 grid gap-3">{FEATURES.map(feature => { const allowed = permissions[feature.key]; const enabled = canShowFeature(account, feature.key); const warningLabel = feature.key === "browser" ? "Investigation Browser" : feature.key === "signals" ? "Workspace Signals" : feature.key === "extractors" ? "Workspace Extractors" : feature.key === "evidence" ? "Workspace Evidences" : feature.key === "sources" ? "Workspace Sources" : "Ultimate Checker"; return <div key={feature.key} className={`rounded-xl border ${allowed ? "border-cyan-400/15 bg-black/40" : "border-white/6 bg-white/[0.015] opacity-40"}`}><div className="flex items-center justify-between gap-4 p-4"><span><span className="block text-[10px] font-black tracking-[0.12em] text-cyan-200/80">{feature.label}</span><span className="mt-1 block text-[10px] text-cyan-100/35">{allowed ? feature.description : "NOT INCLUDED IN THIS TIER"}</span></span><button type="button" role="switch" aria-checked={enabled} aria-label={`${feature.label} ${enabled ? "enabled" : "disabled"}`} disabled={!allowed} onClick={() => { if (!allowed) return; setAccount(prev => ({ ...prev, preferences: { ...prev.preferences, [feature.key]: !enabled } })); setAccountDirty(true); }} className="relative shrink-0 cursor-pointer rounded-full transition-all duration-200 disabled:cursor-not-allowed" style={{ width: 58, height: 32, padding: 2, backgroundColor: !allowed ? "rgba(255,255,255,.10)" : enabled ? "#22d3ee" : "#ef4444", border: `2px solid ${!allowed ? "rgba(255,255,255,.18)" : enabled ? "#67e8f9" : "#f87171"}`, boxShadow: !allowed ? "none" : enabled ? "0 0 18px rgba(34,211,238,.35)" : "0 0 18px rgba(239,68,68,.25)" }}><span className="absolute rounded-full bg-black shadow-md transition-transform duration-200" style={{ width: 24, height: 24, top: 2, left: 2, transform: enabled ? "translateX(26px)" : "translateX(0)" }} /></button></div>{allowed && !enabled ? <p className="border-t border-red-400/10 px-4 py-2 text-[9px] font-black tracking-[0.02em] text-red-300">WARNING: Turning this off will disable the {warningLabel}.</p> : null}</div>; })}</div>
              </div>
              <div className="mt-5 rounded-2xl border border-cyan-400/30 bg-black/70 p-5">
                <div className="border-b border-cyan-400/20 pb-4">
                  <div className="flex items-center justify-between gap-4">
                    <h3 className="text-xl font-black">SIGN OUT</h3>
                    <button type="button" onClick={() => void signOut()} disabled={signingOut} className="shrink-0 rounded-xl px-4 py-3 text-xs font-black bg-red-400 text-black disabled:cursor-not-allowed disabled:opacity-50">{signingOut ? "SIGNING OUT..." : "SIGN OUT"}</button>
                  </div>
                  <p className="mt-2 text-[10px] leading-5 text-cyan-100/45">Signing out of your account? Any unsaved account changes must be saved before you can sign out.</p>
                </div>
              </div>
              
            </>}
          </div>
          <div className="mt-5 text-[10px] font-black tracking-[0.12em] text-cyan-300/60">STATUS: {status}</div>
        </section>
        <footer className="mt-10 border-t border-cyan-400/20 pt-6 text-center text-[10px] font-black tracking-[0.2em] text-cyan-400/40">v1124 ACCOUNT</footer>
      </div>
    </main>
    </>
  );
...
export default function AccountPage(){ return <AuthGate returnTo="/workstation/account"><AccountPageContent /></AuthGate>; }
